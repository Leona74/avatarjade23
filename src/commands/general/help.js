...
module.exports = {
	name: "help",
	description:
		"Generating help commands tab. (specify a category or a command name for more informations for the selected item)",
	category: "general",
	cooldown: 2,
	usage: "help <Str:command_name & category_name>",
	...
	async execute(bot, message, args) {
		...
		const disabledCmds = !guild.disabledCommands[0]
			? [{ category: "disabled", name: "N/A" }]
			: guild.disabledCommands.map((cmd) => {
					return { name: cmd, category: "disabled" };
			  });
		const customCmds = !guild.customCommands[0]
			? [{ category: "custom", name: "N/A" }]
			: guild.customCommands.map((cmd) => {
